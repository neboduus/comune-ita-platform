...
class ProtocolloServiceTest extends AbstractAppTestCase
{
  ...
  public function testProtocolloServiceSendPraticaToInfor()
  {
    $this->markTestSkipped(
      'Infor Protocol handler needs to be refactored to use Guzzle clients (so that they can be mocked)'
    );
    $expectedAllegati = 3;
    $responses = [$this->getInforSuccessResponse()];

    $inforhandler = new InforProtocolloHandler(
      $this->getMockLogger(),
      'a',
      'b',
      'c@c.c',
  ...
  /**
   * @test
   * @expectedException \App\Protocollo\Exception\ResponseErrorException
   */
...
//    }
  ...
  /**
   * @test
   * @expectedException \App\Protocollo\Exception\ParentNotRegisteredException
   */
  ...
  /**
   * @test
   * @expectedException \App\Protocollo\Exception\AlreadyUploadException
   */
  public function testProtocolloServiceCanNotUploadAllegatoTwice()
  {
    $protocollo = $this->getMockProtocollo(
      [$this->getPiTreSuccessResponse(), $this->getPiTreSuccessResponse(), $this->getPiTreSuccessResponse()]
    );
    $user = $this->createCPSUser();
    $pratica = $this->createSubmittedPraticaForUser($user);
    $protocollo->protocollaPratica($pratica);
  ...
  private function getMockProtocollo($responses = array(), $dispatcher = null, $protocollohandlerMock = null)
  {
    ...
    return
      new ProtocolloService(
        $protocollohandlerMock ?? new PiTreProtocolloHandler(
          $this->getMockGuzzleClient($responses),
          'comune-di-tre-ville'
        ),
        $this->em,
        $this->getMockLogger(),
