...
export default class FormioCalendar extends Base {
  ...
  static builderInfo = {
    title: 'Calendar', group: 'basic', icon: 'calendar', weight: 70, schema: FormioCalendar.schema()
  }
  ...
  render(children) {
    ...
    let content = this.renderTemplate('input', {
      input: {
        type: 'input', ref: `${this.component.key}-selected`, attr: {
          id: `${this.component.key}`, class: 'form-control', type: 'hidden',
        }
      }
    });
...
<div id="date-picker-print" class="mt-3 d-print-block d-preview-calendar"></div><div class="mt-3 d-print-none d-preview-none" id="draft-expiration-container"><div id="draft-expiration" class="d-print-none d-preview-none"></div><span id="draft-expiration-countdown" class="font-weight-bolder"></span></div>`);
  ...
  attach(element) {
    ...
    let self = this, calendarID = this.component.calendarId, selectOpeningHours = this.component.select_opening_hours,
      openingHours = this.component.select_opening_hours ? this.component.opening_hours : [],
      location = window.location, html = '', explodedPath = location.pathname.split("/");
    ...
    if (calendarID !== '' && calendarID != null && !this.disabled) {
      ...
      $.ajax(url, {
        ...
        beforeSend: function () {
          ...
          self.calendar = self.container.find('.date-picker').datepicker({
            ...
            onSelect: function (dateText) {
              ...
              if (self.meeting_expiration_time) {
                ...
                $('#draft-expiration').html(`<i>${Translator.trans('calendar_formio.draft_expiration_text', {}, 'messages', self.$language)} ${self.meeting_expiration_time.format("HH:mm")}</i>`)
                $('#draft-expiration-container').addClass('alert alert-info')
                CountDownTimer(self.meeting_expiration_time, 'draft-expiration-countdown')
          ...
          if (self.meeting_expiration_time) {
            $('#draft-expiration').html(`<i>${Translator.trans('calendar_formio.draft_expiration_text', {}, 'messages', self.$language)}</i>`)
          }
        }
      });
  ...
  getFirstAvailableSlot(availabilities) {
    ...
    if (this.first_available_date !== null) {
      let self = this, calendarID = this.component.calendarId, location = window.location,
      ...
      $.ajax(`${location.origin}/${explodedPath[1]}/api/calendars/${calendarID}/availabilities/${this.first_available_date}?available=true`, {
        dataType: 'json', success: function (data, status, xhr) {   // success callback function
          let slot = data[0];
  ...
  getDaySlots() {
    let self = this, calendarID = this.component.calendarId, selectOpeningHours = this.component.select_opening_hours,
      openingHours = this.component.select_opening_hours ? this.component.opening_hours : [], html = '',
      location = window.location, explodedPath = location.pathname.split("/"),
  ...
  createOrUpdateMeeting() {
    let self = this, location = window.location, explodedPath = location.pathname.split("/");
