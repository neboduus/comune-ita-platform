  ...
  var calendar = new Calendar(calendarEl, {
    ...
    eventDrop: function (info) {
      if (
        ...
        info.event.extendedProps.status !== 6
      ) {
        compileModal(info);
        $("#edit_alert").show();
      } else {
    ...
    datesRender: function (info) {
      date_cookie
        ? setCookie("d_date_view", new Date(date_cookie))
        : setCookie("d_date_view", new Date());
    },
  });
...
function compileModal(info) {
  ...
  $("#modalTitle").html(
    `[${getStatus(info.event.extendedProps.status)}] ${
      info.event.extendedProps.name ||
      Translator.trans("meetings.modal.no_name", {}, "messages", language)
    }`
  );
...
$(".modal-edit").on("click", function editMeeting() {
  ...
  if (!start || !end) {
    return $("#modalError").html(
      `<li><span class="badge badge-danger mr-2">${Translator.trans(
        "status_error",
        {},
        "messages",
        language
      )}</span>${Translator.trans(
        "meetings.error.slot_hours_invalid",
        {},
        "messages",
        language
      )}</li>`
  ...
  $.ajax({
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      let error = jqXHR.responseJSON.description
        ? jqXHR.responseJSON.description
        : `${Translator.trans(
            "meetings.error.save_slot",
            {},
            "messages",
            language
          )}`;
      let errorMessage = `<span class="badge badge-danger mr-2">${Translator.trans(
        "status_error",
        {},
        "messages",
        language
