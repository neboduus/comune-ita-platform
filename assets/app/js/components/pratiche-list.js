...
Vue.component("pratiche-list", {
  ...
  methods: {
    submitOperatoreBinding: function (e) {
      ...
      axios
        .post(
          `/amministrazione/api/operatori/${self.chosenOperatore}/ente/${self.chosenEnte}`
        )
        .then(function () {
    ...
    loadEntities: function () {
      const self = this;
      axios.get("/amministrazione/api/operatori").then(function (response) {
      ...
      axios.get("/amministrazione/api/enti").then(function (response) {
        if (response.status == 200) {
...
const renderOperatoreSelectItem = function (item) {
  item.label =
    item.fullName + (item.ente ? ` (${item.enteName})` : ` (nessuno)`);
  return item;
};
