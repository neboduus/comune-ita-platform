 ...
 */
class ServizioCreateCommand extends AbstractCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $serviziRepo = $this->em->getRepository('App:Servizio');
        $categoryRepo = $this->em->getRepository('App:Categoria');

        $servizio = $serviziRepo->findOneBy(['name' => $name]);
        ...
        if (!$servizio instanceof Servizio) {
            ...
            $area = $categoryRepo->findOneBy(['treeId' => $area]);
            ...
            $codiciMeccanograficiEnti = $this->instanceService->getTenant()->getCodiceMeccanografico();

            /** @var Ente[] $enti */
            $enti = $this->em->getRepository('App:Ente')->findBy(['codiceMeccanografico' => $codiciMeccanograficiEnti]);
