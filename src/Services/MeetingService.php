...
class MeetingService
{
    ...
    public function explodeMeetings(OpeningHour $openingHour, DateTime $date)
    {
        ...
        foreach ($periods as $period) {
            ...
            $intervalEnd = clone $intervalBegin;
            $intervalEnd = $intervalEnd->add($meetingInterval);
            if ($intervalEnd <= $end && $shoudAdd) {
                $intervals[$intervalBegin->format('H:i') . '-' . $intervalEnd->modify('- ' . $openingHour->getIntervalMinutes() . ' minutes')->format('H:i') . '-' . $openingHour->getMeetingQueue()] = [
                    'date' => $date->format('Y-m-d'),
                    'start_time' => $intervalBegin->format('H:i'),
    ...
    public function isOverlapped(OpeningHour $openingHour)
    {
        ...
        $openingHours = $this->entityManager->createQueryBuilder()
            ->select('openingHour')
            ->from('App:OpeningHour', 'openingHour')
    ...
    public function sendEmailUpdatedMeeting(Meeting $meeting, $changeSet)
    {
        ...
        $oldDate = null;
        if ($dateChanged) {
            $fromTime = $changeSet['fromTime'][0];
            if ($fromTime instanceof DateTime) {
                $oldDate = $fromTime->format('d/m/Y');
            }
        }
