...
class MyPay extends AbstractPaymentData implements EventSubscriberInterface
{
  ...
  private function generatePaymentButtons( Pratica $pratica, $url )
  {
    ...
    $buttons .= "<p class='mt-5'>".$this->translator->trans('gateway.mypay.redirect_text', ['%gateway_name%' => $pratica->getPaymentType()->getName()])."</p><div class='text-center mt-5'><a href='{$url['url']}' class='btn btn-lg btn-primary'>".$this->translator->trans('gateway.mypay.redirect_button')."</a></div>";
