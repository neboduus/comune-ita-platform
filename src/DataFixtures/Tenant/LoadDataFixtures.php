...
class LoadDataFixtures extends Fixture implements FixtureGroupInterface
{
    use TenantFixturesTrait;
    ...
    public function loadEnti(ObjectManager $manager)
    {
        ...
        foreach ($data as $item) {
            if ($this->instanceService->getTenant() &&
                $item['codice'] != $this->instanceService->getTenant()->getCodiceMeccanografico()) {
                continue;
            }
    ...
    public function loadServizi(ObjectManager $manager)
    {
        ...
        foreach ($data as $item) {
            ...
            if ($this->instanceService->getTenant() &&
                !in_array($this->instanceService->getTenant()->getCodiceMeccanografico(), $codiciMeccanograficiEnti)) {
                continue;
            }
