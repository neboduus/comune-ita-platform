 ...
 * @MustHaveTenant()
 */
class FoldersAPIController extends TenantAwareFOSRestController
{
     ...
     * @return View
     */
    public function putFolder($id, Request $request)
    {
        $repository = $this->getDoctrine()->getRepository('App:Folder');
        $folder = $repository->find($id);
     ...
     * @throws \Exception
     */
    public function patchFolder($id, Request $request)
    {
        $repository = $this->getDoctrine()->getRepository('App:Folder');
        $folder = $repository->find($id);
    ...
    /**
     * Delete a Folder
     * @Rest\Delete("/{id}", name="folders_api_delete", methods={"DELETE"})
     *
     * @SWG\Parameter(
     *     name="Authorization",
     *     in="header",
