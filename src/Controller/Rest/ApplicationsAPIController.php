...
class ApplicationsAPIController extends AbstractFOSRestController
{
    ...
    try {
      if ($request->headers->get('accept') === 'application/geo+json') {
        ...
        foreach ($applications as $a) {
          ...
          $features []= new Feature(new Point([floatval($formData['data']['address']['lon']), floatval($formData['data']['address']['lat'])]));
        }
        $featuresCollection = new FeatureCollection($features);
   ...
   *     @OA\JsonContent(
   *       oneOf={
   *        @OA\Schema(ref=@Model(type=Application::class, groups={"read"})),
   *        @OA\Schema(ref=@Model(type=Point::class)) 
   *      }
   *     )
   *
   * )
  ...
  public function getApplicationAction($id, Request $request)
  {
    ...
    try {
      ...
      if ($request->headers->get('accept') === 'application/geo+json') {
        ...
        $data = new Point([floatval($formData['data']['address']['lon']), floatval($formData['data']['address']['lat'])]);
        $view = new View($data);
        $view->setContext($view->getContext()->setSerializeNull(false));
        return $view;
      } else {
