 ...
 * @MustHaveTenant()
 */
class GiscomAPIController extends TenantAwareController
{
    ...
    /**
     * @var \App\Services\PraticaIntegrationService
     */
    ...
    public function __construct(
        ...
        $this->logger = $logger;
        $this->statusService = $praticaStatusService;
     ...
     * @return JsonResponse
     */
    public function viePratica(Pratica $pratica, GiscomAPIMapperService $mapper)
    {
        return new JsonResponse([
            'pratica' => $mapper->map($pratica),
        ]);
    }
     ...
     * @return Response
     */
    public function attachment(Allegato $attachment)
    {
    ...
    /**
     * @Route("/giscom/pratica/offline/create", name="giscom_api_offline_pratica_create", methods={"GET"})
     * @Security("has_role('ROLE_GISCOM')")
     * @param Request $request
     * @param InstanceService $instanceService
     * @param GiscomAPIAdapterService $giscomAdpterService
     * @return Response
     */
    public function createOfflinePratica(Request $request, InstanceService $instanceService, GiscomAPIAdapterService $giscomAdpterService)
    {
        ...
        try {
            ...
            $user = $this->getDoctrine()
                ...
                ->findOneBy(['username' => $securityUser->getUsername()]);
            ...
            $servizio = $this->getDoctrine()
                ...
                ->findOneBy(['praticaFCQN' => SciaPraticaEdilizia::class]);
     ...
     * @return Response
     */
    public function addStatusChangeToPratica(Request $request, Pratica $pratica)
    {
        $content = $request->getContent();
        if (empty($content)) {
     ...
     * @return Response
     */
    public function addProtocolliToPratica(Request $request, Pratica $pratica)
    {
        $content = $request->getContent();
        if (empty($content)) {
     ...
     * @throws \Exception
     */
    public function createIntegrationRequest(Request $request, SciaPraticaEdilizia $pratica, LoggerInterface $logger)
    {
        $payload = json_decode($request->getContent(), true);
        ...
        $logger->info(LogConstants::RICHIESTA_INTEGRAZIONE_FROM_GISCOM, [
            'id' => $pratica->getId(),
            'request' => $payload
        ]);
